...
在每台 Mac 安装相同版本的 SmartStitch.app，并挂载同一个 NAS Smartstitch 目录。首次启用账号时，在受控电脑的终端运行 `smartstitch --bootstrap-admin` 设置管理员；DMG 安装版可运行 `/Applications/SmartStitch.app/Contents/MacOS/SmartStitch --bootstrap-admin`。若需指定共享配置目录，附加 `--config-directory /path/to/Smartstitch`。随后用管理员账号登录，进入“工具台 → SmartStitch 集群控制”。集群控制和用户管理仅管理员可见，管理员可在用户管理中添加、暂停、恢复或删除用户。所有电脑须升级到支持登录的版本后再开放共享目录，旧版本不会执行新权限校验。

1. 每台电脑打开 App 后自动作为工作机上线。macOS 首次提示局域网访问时允许 SmartStitch 连接；若自动上线失败，可在集群控制面板查看原因并重试。
2. 在固定保持打开的主控电脑上，使用“搜索局域网”点击工作机即可连接，或手动填写地址。局域网连接默认不需要令牌；若工作机手动开启令牌保护，再输入其显示的令牌。主控电脑如需参与渲染，也把本机加入节点列表。
3. 在同一面板选择配置和数量，点击“开始集群渲染”。所有选中素材及成片输出位置都须位于共享 NAS Smartstitch 目录内。面板显示在线节点和批次；任务记录可查看每条成片的进度、工作机和结果。

每台工作机的“任务记录”页另有“本机工作机执行记录”，只列出这台电脑实际接收的渲染或超分条目及其执行状态；完整批次和最终成片结果仍以主控电脑为准。

工作机关闭后不再领取新条目，主控会在心跳超时后把未完成条目派给其他在线工作机。主控在集群运行期间保持打开。各机任务记录仍保存在各自的本机 SQLite，主控通过局域网接口查询工作机状态；NAS 不需要安装数据库。若 Bonjour 搜索不到工作机，可使用其 `.local` 主机名或局域网 IP 与面板显示的端口手动连接。工作机接口使用 HTTP，默认不校验令牌，仅适用于受信任局域网；需要时可在每台工作机上开启令牌保护。跨不可信网络应通过安全隧道连接。详细设计与验收项目见[集群渲染研究](doc/24-多机集群渲染可行性研究.md)。
